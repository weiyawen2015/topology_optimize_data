...
clc
s = pwd;
addpath([s '/fastc/Tensor']);
addpath([s '/fastc/Fast']);
addpath([s '/fastc/Utils']);
addpath([s '/fastc/Scicomp']);
addpath([s '/fastc/Misc']);
...
while ch > 1e-6 && loop < maxit
  ...
  patch(isosurface(isovals, .5),'FaceColor','b','EdgeColor','k');
  patch(isocaps(isovals, .5),'FaceColor','r','EdgeColor','k');
